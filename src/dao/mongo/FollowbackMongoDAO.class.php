...
*/
    ...
    //require_once 'exceptions/Followback/CustomerNotFoundException.class.php';

    class FollowbackMongoDAO implements FollowbackDAO {
        ...
        public function insert($FollowbackObj) {
            global $logger, $warnings_payload;

            $logger -> debug("Insert the customer into `Followback` collection");

            $logger -> debug ("Selecting collection: Followback");
            $this -> mongo -> selectCollection('Followback');


            $logger -> debug("Mongo Customer: " . json_encode($FollowbackObj->toArray() ));
            $result = $this -> mongo -> insert($FollowbackObj->toArray()); 
        ...
        public function insert1($customerObj, $raw = null) {
            ...
            try {
                ...
                if ($duplicate) {
                    ...
                    require_once 'exceptions/Followback/DuplicateCustomerException.class.php';
                    ...
                    if ($conflict) {
                        ...
                        /* Insert the conflicted customer into the `conflictedFollowback` and get it's UUID */
                        ...
                        /* Insert the customer into `Followback` collection */
                        $custToInsert = $customerObj -> serialize();
        ...
        public function deleteByOtherIdentifier($idType, $idValue) {
            ...
            try {
                ...
                /* In case of an entry in the `customer_id_mapping` table but not in the `Followback` collection */
        ...
        public function load($uuidValues, $orgId = null, $projection = null) {
            ...
            $Followback = $output ['result'];
            unset($output ['result']);
            foreach ($Followback as $uuid => $customer) {
        ...
        public function loadByExternalIdentifier($idType, $idValues, $orgId = null, $projection = null) {
            ...
            try {
                if (! empty($mappingOutput ['result'])) {
                    ...
                    $FollowbackOutput = $this -> load($uuids, $orgId, $projection);
                }
                if (! empty($FollowbackOutput ['failures'] ['uuid'])) {
                    foreach ($FollowbackOutput ['failures'] ['uuid'] as $uuid) {
            ...
            return array (
                ...
                'result' => $FollowbackOutput ['result']
            );
        }
        ...
        public function loadAll() {
            ...
            foreach ($mongoServices as $key => $value) {
                ...
                $services[] =  new Followback($value['name'], $value['mobile'], $value['address'], $value ['service'], $value["type"], $value ['timestamp']);
            }
        ...
        public function loadAllInOrderOf($sortByKey) {
            global $logger;
            $Followback = $mongoFollowback = null;

            $logger -> debug ("Selecting collection: Followback");
            $this -> mongo -> selectCollection('Followback');     

            $mongoFollowback = $this -> mongo -> find(array(), array('$sortByKey' => 1));
            foreach ($mongoFollowback as $mongoCustomer) {
                $Followback [] = Customer :: deserialize($mongoCustomer);
            }
        ...
        public function loadCustomerInConflict($conflictCustomerUuid) {
            ...
            $logger -> debug ("Selecting collection: conflictFollowback");
            $this -> mongo -> selectCollection('conflictFollowback');     
        ...
        public function loadAllFollowbackInConflict() {
            global $logger;
            $FollowbackInConflict = $mongoFollowbackInConflict = null;

            $logger -> debug ("Selecting collection: conflictFollowback");
            $this -> mongo -> selectCollection('conflictFollowback');     

            $mongoFollowbackInConflict = $this -> mongo -> find(array());
            foreach ($mongoFollowbackInConflict as $mongoCustomerInConflict) {
                $FollowbackInConflict [] = Customer :: deserialize($mongoCustomerInConflict);
            }
            
            return $FollowbackInConflict;
        }
        ...
        public function insertConflict($conflictUuid, $conflictCases) {
            ...
            global $logger;    
            $logger -> debug("Insert the conflicted customer into `conflictedFollowback` and get it's UUID");
        ...
        public function insertRawdata($raw) {
            ...
            global $logger;    
            $logger -> debug("Insert the raw data into the 'FollowbackInput' collection");

            $this -> mongo -> selectCollection('FollowbackInput');
